...
Extension code in `extensions/` and tests in `tests/` are managed with Deno. A `deno.json` at the repo root scopes fmt, lint, and test to these directories and defines tasks with the correct arguments.
...
# Run tests (requires SSH connectivity to test nodes)
deno task test
```

**Rule:** All three checks (`deno task fmt:check`, `deno task lint`, and `deno task test`) must pass before considering extension work complete.
